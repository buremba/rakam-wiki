...
They provide you the tools seperately, you need to glue them together and build your own solution on top of Snowplow.
...
instead you choose your database and make them to send the data to it. Currently, they support Redshift, Postgresql and Elasticsearch and we provive two solutions; 
Postgresql and PrestoDB. Postgresql is flexible, real-time and fast for small and medium size data volumes but unfortunately not horizontaly scalable. 
...
Since ad-hoc queries are not enough for dashboards, we implemented stream processing inside the database 
with continuous queries, just write the SQL query and create continuous query background job. 
Rakam will process the data at ingestion time so you don't need to process billions of data when visiting your dashboards.
...
if the data volume is large and usually run queries on small part of the data-set (last month, last day etc.),
...
on larger data-sets and close them when they don't need. It affects your bills at the end of the month because you will be allocating the real resource 
you will actually consume on the cloudd.
...
and even allow you to implement custom event mappers with Javascript at runtime. For example, you can add Clearbit integration
with just one click (Since we provide boilerplate for some of the third party services) or implement support for similar third party service with 10 lines of JS code. 
It just get an attribute from the event, perform a HTTP request and attach some values to the event.
...
If you already have BI solution that you're familiar with, you can also plug it to Rakam API.
...
Big-data world doesn't need to be hard to get it right.
